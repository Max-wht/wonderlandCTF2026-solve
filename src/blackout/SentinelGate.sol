...
contract SentinelGate {
    ...
    fallback() external payable {
        ...
        //0x40 =
        assembly {
            mstore(0x00, calldataload(4))  // 0x00[address to]0x14[blank]0x20[blacklist.slot]0x40 
            mstore(0x20, blacklisted.slot)
            _isBlacklisted := sload(keccak256(0x00, 0x40)) //read from map,
        }
        if (_isBlacklisted) revert SentinelGate_Blacklisted();

        address _to;
        assembly {
            _to := calldataload(4) //0x00[address to]0x20
        }
