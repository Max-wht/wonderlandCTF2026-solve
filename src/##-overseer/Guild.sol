...
contract Guild is OverseerEye, IFolk {
  ...
  constructor(address _player, IOverseer _overseer, CouncilRules memory _initialRules, address[] memory _initialElders)
  ...
  function write(bytes16 _fromBadge, bytes32 _activity, bytes32 _subject, bytes calldata _data) external override {
    // only overseer can call this function
    if (!hasRank(OVERSEER_ROLE, msg.sender)) revert Trespasser();
    // record the folk but not the badge
  ...
  function getDecreeInfo(bytes16 _decreeId)
    public
    view
    returns (
      bool _enacted,
