...
import {Overseer} from "./Overseer.sol";
import {Guild} from "./Guild.sol";
import {SealedTurncloak} from "./elders/SealedTurncloak.sol";
import {Loyalist} from "./elders/Loyalist.sol";
...
contract Challenge {
  ...
  constructor(address player) payable {
    ...
    guild = new Guild(
      player, IOverseer(address(overseer)), Guild.CouncilRules({verdictThreshold: 3, duration: 15}), initialElders
    );
