...
contract TransactionCapsule {
  ...
  constructor(address _wallet, address _target, bytes4 _selector, uint256 _maxValue, address _guard) {
    wallet = _wallet;
    target = _target;
    selector = _selector;
    maxValue = _maxValue;
  ...
  function validate(address _wallet, uint256 _value) external returns (bool) {
...
contract CannonGuard {
  ...
  // a wallet hold lots of capsules?
  mapping(address => address[]) internal _walletCapsules;
  mapping(address => bool) public isCapsule;

  event CapsuleCreated(
    address indexed wallet, address indexed capsule, address target, bytes4 selector, uint256 maxValue
  );
  ...
  function createCapsule(address wallet, address target, bytes4 selector, uint256 maxValue)
    external
    returns (address capsuleAddr)
  {
    require(msg.sender == commander, "Only commander");

    TransactionCapsule capsule = new TransactionCapsule(wallet, target, selector, maxValue, address(this));
  ...
  // so once the capsule created, anyOne can call it. The Commander have lots of power yeah.
  ...
  function validateAndConsume(address capsule, uint256 value) external returns (bool) {
