...
contract SafeSmartWallet {
  ...
  function execTransaction(address to, uint256 value, bytes calldata data) external onlyOwner returns (bool) {
  ...
  function executeApprovedCapsule(address capsuleAddress, bytes calldata params) external payable returns (bool) {
    ...
    bool valid = ICannonGuard(guard).validateAndConsume(capsuleAddress, msg.value);
