...
abstract contract PoolLiquidation is PoolState {
  ...
  function liquidate(address collateralAsset, address debtAsset, address user, uint256 amount)
    external
    virtual
    invariants(user)
  {
    require(
      reserves[collateralAsset].underlying != address(0) && reserves[debtAsset].underlying != address(0),
      "asset not listed"
    );
  ...
  function invariant(address user) internal virtual override {
