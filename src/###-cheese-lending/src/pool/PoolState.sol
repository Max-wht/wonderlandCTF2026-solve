...
abstract contract PoolState {
  ...
  mapping(address => uint256) public price; // fixed price in common unit: Gruyere=1, Emmental=2

  mapping(address => mapping(address => uint256)) public userSupplyBalance; // record one collateral amount
  mapping(address => mapping(address => uint256)) public debtPrincipal; // this record the user's debt
  mapping(address => mapping(address => uint256)) public debtLastUpdate; // this record the lastest debt
  ...
  function _initReserve(address asset, uint256 assetPrice) internal {
    ...
    reserves[asset] = ReserveData({underlying: asset, totalSupplied: 0, totalDebt: 0});
  ...
  function _accrueDebt(address user, address asset) internal {
    uint256 increase = getDebtIncrease(user, asset);
